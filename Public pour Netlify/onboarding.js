        ...
        <div id="onboardingOverlay" class="onboarding-overlay">
            <div class="onboarding-modal">
                ...
                <div class="mode-selector">
                    <button type="button" class="mode-btn active" data-mode="flash" onclick="setOnboardingMode('flash')">
                        ⚡ Mode Flash <span class="mode-hint">3 questions</span>
                    </button>
                    <button type="button" class="mode-btn" data-mode="expert" onclick="setOnboardingMode('expert')">
                        🎯 Mode Expert <span class="mode-hint">Profil complet</span>
                    </button>
                </div>
                ...
                <form id="onboardingForm" class="onboarding-form">
                    ...
                    <div class="onboarding-section section-flash">
                        <h3>👤 Ton identité</h3>
                        ...
                        <div class="form-group expert-only" style="display: none;">
                    ...
                    <!-- SECTION 2 : Audience - EXPERT ONLY -->
                    <div class="onboarding-section expert-only" style="display: none;">
                        <h3>🎯 Ton audience</h3>
                    ...
                    <!-- SECTION 3 : Contenu - EXPERT ONLY -->
                    <div class="onboarding-section expert-only" style="display: none;">
                        <h3>📝 Ton contenu</h3>
                        
                        <div class="form-group">
                            <label for="onb-piliers">Tes piliers de contenu (2-4 thématiques)</label>
                            <input type="text" id="onb-piliers" placeholder="Ex: mindset, business, lifestyle" 
                    ...
                    <!-- SECTION 4 : Plateformes - FLASH (simplifié) -->
                    <div class="onboarding-section section-flash">
                        <h3>📱 Tes plateformes</h3>
                        
                        <div class="form-group">
                            <label>Où publies-tu ?</label>
                            <div class="checkbox-grid checkbox-grid-compact">
                                ${ONBOARDING_OPTIONS.plateformes.slice(0, 6).map(p => `
                                    <label class="checkbox-card">
                    ...
                    <!-- SECTION 4b : Plateformes complètes - EXPERT ONLY -->
                    <div class="onboarding-section expert-only" style="display: none;">
                        <h3>📱 Plus de plateformes & formats</h3>
                        
                        <div class="form-group">
                            <label>Autres plateformes</label>
                            <div class="checkbox-grid checkbox-grid-compact">
                                ${ONBOARDING_OPTIONS.plateformes.slice(6).map(p => `
                                    <label class="checkbox-card">
                                        <input type="checkbox" name="plateformes" value="${p.value}" 
                                               ${existingProfile?.plateformes?.includes(p.value) ? 'checked' : ''}>
                                        <span class="checkbox-content">
                    ...
                    <!-- SECTION 5 : Objectif - FLASH -->
                    <div class="onboarding-section section-flash">
                        <h3>🎯 Ton objectif principal</h3>
                        
                        <div class="form-group">
                            <div class="radio-cards">
                                ${ONBOARDING_OPTIONS.objectifs.slice(0, 4).map(o => `
                    ...
                    <!-- SECTION 5b : Objectifs complets + Profil créateur - EXPERT ONLY -->
                    <div class="onboarding-section expert-only" style="display: none;">
                        <h3>🎯 Plus d'objectifs</h3>
                        
                        <div class="form-group">
                            <div class="radio-cards">
                                ${ONBOARDING_OPTIONS.objectifs.slice(4).map(o => `
                                    <label class="radio-card">
                                        <input type="radio" name="objectif" value="${o.value}" 
                                               ${(existingProfile?.objectif || '') === o.value ? 'checked' : ''}>
                                        <span class="radio-content">
                        ...
                        <div class="form-group">
                            <label for="onb-problematique">Ta problématique actuelle</label>
                            <textarea id="onb-problematique" placeholder="Ex: Je n'arrive pas à être régulier, Je ne sais pas quoi poster, Mon contenu n'engage pas..." 
                                   style="min-height: 60px;">${existingProfile?.problematique || ''}</textarea>
                        </div>
                    </div>
                    
                    <!-- SECTION 6 : Profil créateur - EXPERT ONLY -->
                    <div class="onboarding-section expert-only" style="display: none;">
                                ...
                                ${ONBOARDING_OPTIONS.niveaux.map(n => `
                                    <label class="radio-card">
                                        ...
                                        <span class="radio-content">
                                            ...
                                            <span class="radio-description">${n.description}</span>
                                        </span>
                                    </label>
                                `).join('')}
                            </div>
                    ...
                    <!-- SECTION 7 : Précisions - EXPERT ONLY -->
                    <div class="onboarding-section expert-only" style="display: none;">
                        <h3>💬 Autre chose ?</h3>
...
/**
 * Bascule entre mode Flash et mode Expert
 */
function setOnboardingMode(mode) {
    ...
    // Show/hide expert sections
    const expertSections = document.querySelectorAll('.expert-only');
    expertSections.forEach(section => {
        section.style.display = mode === 'expert' ? 'block' : 'none';
    });
}
