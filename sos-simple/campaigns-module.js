...
const CampaignsModule = {
        ...
        return `
            <div class="wizard-step-content">
                ...
                <div class="prospect-filter-options">
                    ...
                    <label class="radio-card ${salesNavCount === 0 ? 'disabled' : ''}">
                        <input type="radio" name="prospectFilter" value="sales_navigator" ${salesNavCount === 0 ? 'disabled' : ''}>
                        <div class="radio-content">
                            <strong>💼 Sales Navigator</strong>
                            <span class="count">${salesNavCount}</span>
                        </div>
                    </label>
                </div>
                ...
                ` : `
                    <div class="selected-prospects-preview">
                        <h4>📋 Prospects sélectionnés (${prospects.length})</h4>
    ...
    async saveStep2() {
        try {
            const filterInput = document.querySelector('input[name="prospectFilter"]:checked');
            const filter = filterInput ? filterInput.value : 'all';
            let prospects = ProspectsModule?.prospects || [];

            console.log('saveStep2 - filter:', filter, 'prospects count:', prospects.length);

            if (filter === 'new') {
                prospects = prospects.filter(p => p.status === 'new');
            } else if (filter === 'sales_navigator') {
                prospects = prospects.filter(p => p.source === 'linkedin_extension' || p.source === 'linkedin_sales_navigator');
            }

            if (prospects.length === 0) {
                alert('Aucun prospect disponible. Veuillez d\'abord importer des prospects.');
                return;
            }
    ...
    renderStep4_Emails() {
        return `
            <div class="wizard-step-content">
                ...
                <p class="step-description">L'IA va générer des emails personnalisés pour chaque prospect.</p>
